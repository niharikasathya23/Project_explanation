...
I also completed my **independent study** and worked as a **Research Assistant at the HIRO LAB** in CU, where I worked on developing a lightweight neural network for real-time spatial perception called **Bilateral Segmentation and Disparity Refinement (BSDR)**.  The goal was to design a model that could handle large-scale 3D data efficiently and run reliably on resource-constrained hardware. I focused on optimizing architecture by combining segmentation and depth refinement in one model, cutting compute load by over 80% while improving accuracy in tasks like human-robot handovers. That experience taught me how to design systems that balance performance, scalability, and resource efficiency — the same principles I’d bring to building cloud infrastructure for AI and robotics at Trossen.
...
"Earlier at Rakuten, I contributed to their B2B order management platform, working across AngularJS, Spring Boot, and MySQL. I helped implement CI/CD pipelines, resolved critical bugs, and redesigned UI components — boosting engagement by 30% and reducing development cycles by 25%."
...
I recently completed my Master’s in Computer Science at CU Boulder, where I focused on distributed systems, big data, and backend engineering. That gave me a strong theoretical foundation in how large-scale systems are designed and optimized, and I was able to apply a lot of that in research and industry projects. Right now, I’m working as a Software Engineer at Virufy. The big challenge there is making machine learning models usable in the real world. My role has been to design and deploy backend services that connect mobile and web clients to inference modules. For example, when someone uploads a cough audio file, that request goes through the backend API I built — it packages the data, runs it against our CNN-LSTM models, and returns results in real time. I developed those APIs in Python and Java, containerized them with Docker, and deployed them on AWS ECS and Lambda using Terraform. Since speed was critical for user experience, I tuned the APIs to keep inference latency under 300 milliseconds. And because we handle sensitive health data, security was just as important — I integrated OAuth2 and JWT authentication across the API layer, enforced SSL/TLS encryption, and made sure everything was compliant with HIPAA-like privacy standards. I also automated deployments using GitHub Actions, which cut our release cycle time by about 40 percent. That experience really gave me confidence in writing clean, testable code, thinking about security upfront, and deploying reliably in the cloud. Before Virufy, I worked as a Research Assistant at CU Boulder’s HIRO Lab. My focus was real-time robotics perception, which at its core is another backend data problem. I designed distributed pipelines in C++ and Python, using Kafka and Spark, to process huge volumes of stereo camera and sensor data in real time. The system had to run at 60 frames per second for simulations, so performance and reliability were non-negotiable. I learned how to optimize APIs and backend data services under high throughput conditions, and it really sharpened my skills in distributed systems, event-driven processing, and database-backed APIs. Earlier, I interned at Rakuten. There, I contributed to their B2B order management portal, which is a high-traffic system used by businesses. I worked with Spring Boot, AngularJS, and MySQL to build new features and enhance performance. I also fixed over 35 high-priority bugs, which reduced post-release defects by 20 percent and improved customer experience. That gave me a lot of hands-on exposure to relational databases, debugging production issues, and working in an agile environment with product and QA teams. And before that, I interned at DRDO, where my work was focused on secure data-sharing systems. I built REST APIs with PostgreSQL and React, and a lot of my focus there was on encryption and access control. I implemented OAuth2 flows, JWT-based authentication, and SSL/TLS encryption to protect sensitive defense data. I also worked with Jenkins CI/CD pipelines, which gave me early exposure to automation and collaborative workflows in a defense-grade environment. Across these roles, I’ve consistently built backend systems using Node.js, Python, and Java, worked with relational databases like PostgreSQL and MySQL, and deployed on cloud platforms like AWS and GCP. I’ve containerized services with Docker, automated pipelines with Terraform and GitHub Actions, and used Git for version control in collaborative teams. I’ve also had exposure to distributed data systems like Kafka and Spark, which I think is valuable when thinking about scaling and reliability. While I haven’t yet built blockchain systems in production, the work I’ve done maps closely to what D3 is building — secure, scalable APIs, real-time services, and cloud deployments that bridge different platforms together. What excites me about D3 is the chance to bring those backend fundamentals into a zero-to-one environment and learn the blockchain side from your experienced team, while contributing from day one to building the registrar and marketplace for DomainFi.” write a read me for this
